...
struct node *recursive_search_ops(struct node *ptr,int val)
{
    if(ptr==NULL)
    {
        return NULL;
    ...
    {
        return recursive_search_ops(ptr->right,val);
    }
    else
    {
        return recursive_search_ops(ptr->left,val);
    }
}
...
void main()
{
    ...
    int search=3;
    struct node *d=recursive_search_ops(p,search);
    if(d != NULL)
    {
        printf("\nFOUND THE ELEMENT\n");
    }
